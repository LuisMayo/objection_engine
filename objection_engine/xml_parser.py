...
class Handler(xml.sax.ContentHandler):
    ...
    def startElement(self, name: str, attrs: xml.sax.xmlreader.AttributesImpl):
        ...
        # print(f"startElement {name=} {attrs_dict=}")
        if name == "page":
            ...
            self.__current_page_contents.append(
                DialogueAction(f"sprite {position} {src}", 0)
            )
        elif name == "wait":
            ...
            self.__current_page_contents.append(
                DialogueAction(f"bubble {type} {character}", 0)
            )
            ...
            self.__current_page_contents.append(
                DialogueAction(f"nametag {name_text}", 0)
            )
            ...
            if action == "clear":
                self.__current_page_contents.append(
                    DialogueAction(f"evidence clear", 0)
                )
            else:
                src = attrs_dict.get("src")
                if src is None:
                ...
                self.__current_page_contents.append(
                    DialogueAction(f"evidence {action} {src}", 0)
                )
            ...
            self.__current_page_contents.append(
                DialogueAction(f"shake {magnitude_str} {duration_str}", 0)
            )
            ...
            self.__current_page_contents.append(
                DialogueAction(f"flash {duration_str}", 0)
            )
            ...
            if action not in ["start", "stop"]:
                raise Exception(
                    '`<music/>` tag\'s `action` attribute must be `"start"` or `"stop"`'
                )
            ...
            if action == "start":
                ...
                self.__current_page_contents.append(
                    DialogueAction(f"music start {src}", 0)
                )
            elif action == "stop":
            ...
            if action == "set":
                ...
                if new_text is None:
                    raise Exception(
                        '`<verdict action="set"/>` tag needs `text` attribute'
                    )
                color = attrs_dict.get("color")
                if color is None:
                    raise Exception(
                        '`<verdict action="set"/>` tag needs `color` attribute'
                    )
                self.__current_page_contents.append(
                    DialogueAction(f'verdict set "{new_text}" {color}', 0)
                )
            elif action == "show":
                index = attrs_dict.get("index")
                if index is None:
                    raise Exception(
                        '`<verdict action="show"/>` tag needs `index` attribute'
                    )
                self.__current_page_contents.append(
                    DialogueAction(f"verdict show {index}", 0)
                )
            elif action == "clear":
                self.__current_page_contents.append(DialogueAction("verdict clear", 0))
            else:
                raise Exception(
                    '`<verdict/>` tag\'s `action` attribute must be `"set"`, `"show"`, or `"clear"`'
                )
